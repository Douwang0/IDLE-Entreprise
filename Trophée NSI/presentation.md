# Présentation du modèle

La présentation du projet est une description synthétique rédigée par les élèves. Vous trouverez ci-dessous les informations à détailler dans ce fichier : Un simulateur d'entreprise à l'humour très présente, gérez des marchés et des employés très puissants avec des évènements aléatoires.

## Présentation globale

*Vous présentez le contexte de réalisation de votre projet => compétences évaluées : pertinence et son degré d'utilité de votre projet, originalité des idées.*

Précisez les informations suivantes :
- naissance de l’idée, la volonté de faire un jeu vidéo en python sans librairie trop complexe.
- problématique initiale, créer un logiciel utilisant la programmation orientée objet
- objectifs de la solution proposée. Créer un simulateur de gestion d'entreprise avec interface graphique, a titre humoristique

## Présentation de l'équipe

- Charles : Lead Programmeur full-stack, Dev GUI -> Toute l'interface graphique -> beaucoup trop de temps
- Giulian : Chef de projet, Back-end -> Programmation de quelques fonctionnalité et management de l'équipe -> un grand nombre d'heure
- Lylian : Programmeur full-stack, directeur du departement de l'humour -> Programmation de quelques fonctionnalité et beaucoup d'idée -> quelques heures
- Evan : Designer GUI -> Tous les graphismes -> quelques heures

## Étapes du projet

- Naissance du projet après brainstorming
- partage des roles
- début du code pour Lylian, Giulian et Charles
- Attente de l'interface de Charles -> Lylian et Giulian bloqués
- Interface terminé -> Evan peut dessiner les élements, tests possibles pour Lylian et Giulian
- Refonte du code qui ne fonctionnait pas à cause du manque de test
- Projet terminé

## Validation de l’opérationnalité et du fonctionnement 

- Notre projet fonctionne plutôt bien, les différentes fonctionnalités sont implémentées avec succès, quelques évènements on des problèmes mais dans la grande majorité tout va bien.
- Il n'y a normalement pas de moyen de casser le jeu, pas de bug majeur qui va le faire crasher donc amusez vous !
- On a eu pour problème l'organisation du code, l'équilibrage économique, la gestion des événements et le liens entre logique et interface, et finalement on les a réglé en reprenant le code au dernier moment.

## Ouverture 

*Vous réalisez un bilan personnel sur ce projet et détaillez l'expérience vécue par chaque membre de l'équipe => compétences évaluées : analyse critique, prise de recul, implication dans le projet.*

Précisez les informations suivantes :
- idées d'amélioration du projet,
- analyse critique, 
- compétences personnelles développées.
- démarche d'inclusion.

Ceci est un MODELE_PRESENTATION.
